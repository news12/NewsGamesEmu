...
begin
  ...
  Table := FunctionBase.SelectTable(TableCharacter, 'c.*,' + 's.level,' +
    's.defence,' + 's.attack,' + 's.merchant,' + 's.speed,' + 's.inu1,' +
    's.inu2,' + 's.maxHP,' + 's.maxMP,' + 's.HP,' + 's.MP,' + 's.s_str,' +
    's.s_int,' + 's.s_dex,' + 's.s_con,' + 's.wMaster,' + 's.fMaster,' +
    's.sMaster,' + 's.tMaster', '', eSQL, ' c');
  ...
  begin
    ...
    Try
      ...
      ByteArray := FunctionBase.GetByte(Table.FieldByName('name').AsString);
      for p := 0 to 11 do
      ...
      logStr := 'o Jogador: ' + result.User + ' - Entrou no Jogo: ' +
        FunctionBase.GetString(Player.Character[I].Name, 12);
...
begin
  ...
  Table := FunctionBase.SelectTable(TableCharacter, '*', '', eSQL);
  Table.Last;
  X := Table.RecordCount;
  if X < MaxChar then
  begin
    Try
      ...
      Table.FieldByName('name').AsString :=
        FunctionBase.GetString(Player.Character[CharID].Name, 12);
    ...
    Except
      On E: Exception do
      begin
        logStr := 'Erro ao criar o Personagem :[' + FunctionBase.GetString
          (Player.Character[CharID].Name, 12) + '] ' + E.Message;
        // DM.Transaction.Rollback;
        result := False;
      end;
    End;
